...
static double getConditionalProbability(lscde *model,double y,double *X,double from,double to){
	int i,n;
...
static void getLocalScore(int numValuePattern,int numNode,int currentNode,int numLine,Associate *cHist,unsigned short parentPattern,double *scores,Associate **CPT,int numSample,int rank,int start){
	...
	if(numLine==rank){
		for(i=0;i<dimention;i++){
			X[i]=data[i*dimantion+parentPattern];
			Y[i]=data[i*dimantion+currentNode];
		}
		lscde *model=lscdeTrain(Y,X);
